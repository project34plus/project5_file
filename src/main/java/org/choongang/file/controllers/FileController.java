...
public class FileController {
    ...
    @ApiResponse(responseCode = "200") //void일 경우 200 나옴
    @Parameters({
            @Parameter(name = "gid", required = true, description = "경로 변수, 그룹 ID"),
            @Parameter(name = "location", description = "파일 그룹내 위치", example = "editor")
    })
    @GetMapping("/done/{gid}")
    public void processDone(@PathVariable("gid") String gid, @RequestParam(name = "location", required = false) String location) { // 파일 그룹 작업 완료 시 요청 보낼 주소
